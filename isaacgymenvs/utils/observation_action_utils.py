...
# =============== ROBOT CUSTOM CONFIG (JOINT AND LINK) ===============
...
PALM_LINK_NAME = "left_wrist_yaw_link"
...
NUM_FINGERS = len(FINGERTIP_LINK_NAMES)

ACTION_JOINT_INDEX = [
    0, 1, 2, 3, 4, 5, 6,    # arm joints
    7, 8, 10, 12, 14, 16,   # hand joints (skip mimic joints 9, 11, 13, 15, 17)
]
MIMIC_JOINT_MAP = {
    9: (8, 2.29),
    11: (10, 0.89),
...
assert Q_LOWER_LIMITS_np.shape == (NUM_DOFS,), (
    f"Q_LOWER_LIMITS_np.shape: {Q_LOWER_LIMITS_np.shape}, expected: ({NUM_DOFS},)"
)
assert Q_UPPER_LIMITS_np.shape == (NUM_DOFS,), (
    f"Q_UPPER_LIMITS_np.shape: {Q_UPPER_LIMITS_np.shape}, expected: ({NUM_DOFS},)"
)

Q_LOWER_LIMITS_restricted_np = Q_LOWER_LIMITS_np.copy()
Q_LOWER_LIMITS_restricted_np[:DOF_CONFIG["arm"]] += np.deg2rad(10.0)

Q_UPPER_LIMITS_restricted_np = Q_UPPER_LIMITS_np.copy()
Q_UPPER_LIMITS_restricted_np[:DOF_CONFIG["arm"]] -= np.deg2rad(10.0)
assert Q_LOWER_LIMITS_restricted_np.shape == (NUM_DOFS,), (
    f"Q_LOWER_LIMITS_restricted_np.shape: {Q_LOWER_LIMITS_restricted_np.shape}, expected: ({NUM_DOFS},)"
)
assert Q_UPPER_LIMITS_restricted_np.shape == (NUM_DOFS,), (
    f"Q_UPPER_LIMITS_restricted_np.shape: {Q_UPPER_LIMITS_restricted_np.shape}, expected: ({NUM_DOFS},)"
)
...
N_OBS = (
    NUM_DOFS            # joint_pos
    + NUM_DOFS          # joint_vel
    + NUM_DOFS          # prev_action_targets
    + 3                 # palm_pos
    + 4                 # palm_rot
    + 4                 # object_rot
    + 3 * NUM_FINGERS   # fingertip_pos_rel_palm
...
# 掌心相对于手臂与灵巧手的连接处原点(PALM_LINK)的偏移, 偏移量根据URDF+可视化估算, 坐标轴根据PALM_LINK定义
PALM_OFFSET_np = np.array(
    [0.105, 0.00, 0.00],
    dtype=np.float32,
)

# 指尖相对于指尖关节原点(FINGERTIP_LINK_NAMES)的偏移, 坐标轴同理
...
def create_urdf_object(
    robot_name: Literal["g1_29dof_left_linkerhand_adjusted"],
) -> yourdfpy.URDF:
...
def compute_fk_dict(
    ...
    N = q.shape[0]
    assert q.shape == (N, NUM_DOFS), f"q.shape: {q.shape}, expected: (N, {NUM_DOFS})"
...
def compute_joint_pos_targets(
    ...
    cur_targets[:, :arm_dof] = (
        prev_targets[:, :arm_dof] + hand_dof_speed_scale * dt * actions[:, :7]
    )
    cur_targets[:, :arm_dof] = tensor_clamp(
        cur_targets[:, :arm_dof],
        q_lower_limits[:arm_dof],
        q_upper_limits[:arm_dof],
    )
    cur_targets[:, :arm_dof] = (
        arm_moving_average * cur_targets[:, :arm_dof]
        + (1.0 - arm_moving_average) * prev_targets[:, :arm_dof]
    )
...
def _compute_palm_center_pos_and_rot(
    ...
    if PRINT_TIMING:
        ...
        print("t02 - t01: {(t02 - t01) * 1000:.1f} ms, {(t02 - t01) / total_dt * 100:.1f}%")
...
def _compute_fingertip_positions_with_offsets(
    ...
    N = T_R_Fs.shape[0]
    assert T_R_Fs.shape == (N, NUM_FINGERS, 4, 4), (
        f"T_R_Fs.shape: {T_R_Fs.shape}, expected: (N, NUM_FINGERS, 4, 4)"
    )
    ...
    ).reshape(N, NUM_FINGERS, 4)

    fingertip_offsets = FINGERTIP_OFFSETS_np[None].repeat(N, axis=0)
    assert fingertip_offsets.shape == (N, NUM_FINGERS, 3), (
        f"fingertip_offsets.shape: {fingertip_offsets.shape}, expected: (N, NUM_FINGERS, 3)"
    )
    fingertip_positions_with_offsets = np.zeros((N, NUM_FINGERS, 3), dtype=np.float32)
